...
# Qualys API URLs - See https://www.qualys.com/platform-identification/
...
Invoke-RestMethod "$DS_baseUrl/api/computers/$DS_hostId/intrusionprevention/rules" -Headers $DS_headers | ConvertTo-Json | Select-String -Pattern "(?<=`"CVE`":\s*`")((.|\n)*?)(?=`")" -AllMatches | % { $_.Matches.Value } > ds_cve.txt
(get-content ds_cve.txt) -join " " | foreach-object {$_ -replace " ", ","} | set-content ds_cve.txt
$DS_vPatch=get-content ds_cve.txt

# Querying the QIDs of vulnerabilities detected by Qualys on this host and saving them in q_qid.txt
Invoke-WebRequest -Uri "$Q_qualysapiUrl/api/2.0/fo/asset/host/vm/detection/?action=list&ids=$Q_hostId" -UseBasicParsing -Headers $Q_headers1 | select-String -Pattern '(?<=<QID>)[^<]+' -AllMatches | % { $_.Matches.Value } > q_qid.txt
(get-content q_qid.txt) -join " " | foreach-object {$_ -replace " ", ","} | set-content q_qid.txt
$Q_qid=get-content q_qid.txt

# Querying the CVEs associated to the QIDs detected in this host and saving them in q_cve.txt
Invoke-WebRequest -Uri "$Q_qualysapiUrl/api/2.0/fo/knowledge_base/vuln/?action=list&ids=$Q_qid" -UseBasicParsing -Headers $Q_headers1 | select-String -Pattern '(?<=<ID><!\[CDATA\[)CVE-\d+[^]]+' -AllMatches | % { $_.Matches.Value } > q_cve.txt
(get-content q_cve.txt) -join " " | foreach-object {$_ -replace " ", ","} | set-content q_cve.txt

# Identifying CVEs detected by Qualys that are already being protected with Deep Security
$q_cve_tmp = Get-Content q_cve.txt | % { $_ -split ',' } | Select-Object -Unique
$ds_cve_tmp = Get-Content ds_cve.txt | % { $_ -split ',' } | Select-Object -Unique
Compare-Object $q_cve_tmp $ds_cve_tmp -IncludeEqual -ExcludeDifferent | % { $_.InputObject } > ds_protected.txt

$DS_pCount = Get-Content ds_protected.txt | Measure-Object -Line | Select-Object -ExpandProperty Lines
(get-content ds_protected.txt) -join " " | foreach-object {$_ -replace " ", ","} | set-content ds_protected.txt
$DS_protected = Get-Content ds_protected.txt
...
$DS_response | Select-String -Pattern '(?<="ID":\s*)[^,]*' | Select-Object -ExpandProperty Matches | Select-Object -ExpandProperty Value | Out-File -FilePath ds_newVP.txt
}
...
$DS_newVPCount = Get-Content ds_newVP.txt | Measure-Object -Line | Select-Object -ExpandProperty Lines
Write-Output "$DS_newVPCount Virtual Patching rules will be added to this host"
Write-Output ""

(Get-Content ds_newVP.txt) -replace '\s+' | Out-File ds_newVP.txt
(Get-Content ds_newVP.txt) -join " " | foreach-object {$_ -replace " ", ","} | set-content ds_newVP.txt

$DS_newVP = Get-Content ds_newVP.txt
